...
void display_lcd(String message) {
  Serial.println(F("Displaying message: " + message));
  lcd.init();
  lcd.clear();
  lcd.backlight();
  //lcd.home();
...
int parse_raw(int user_index) {
  ...
  if(stories[user_index] != i_id){
    ...
    Serial.print(F("latest story id:"));
    Serial.println(i_id);
    Serial.print(F("story points:"));
    Serial.println(i_estimate);
    
    stories[user_index] = i_id;
    return i_estimate;
  } else {
    Serial.println(F("No new stories found"));
    return 0;
  }
}
